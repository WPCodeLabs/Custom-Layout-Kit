<?php
/**
 * API Manager handles registering actions and hooks with the WordPress Plugin API.
 *
 * Defines wrapper functions for add_action and add_filter
...
class API {
	...
	public function register( $object ) {
		// Register Actions
		if ( $object instanceof \WPCL\CLK\Interfaces\Action_Hook_Subscriber ) {
			$this->register_actions( $object );
		}
		// Register Filters
		if ( $object instanceof \WPCL\CLK\Interfaces\Filter_Hook_Subscriber ) {
			$this->register_filters( $object );
		}
		// Register Shortcodes
		if ( $object instanceof \WPCL\CLK\Interfaces\Shortcode_Hook_Subscriber ) {
	 ...
	 */
	private function register_action( \WPCL\CLK\Interfaces\Action_Hook_Subscriber $object, $name, $parameters ) {
		// For string params
	 ...
	 */
	private function register_actions( \WPCL\CLK\Interfaces\Action_Hook_Subscriber $object ) {
	 ...
	 * @param mixed                           $parameters
	 */
	private function register_filter( \WPCL\CLK\Interfaces\Filter_Hook_Subscriber $object, $name, $parameters ) {
	 ...
	 * @param Filter_Hook_SubscriberInterface $object
	 */
	private function register_filters( \WPCL\CLK\Interfaces\Filter_Hook_Subscriber $object) {
	 ...
	 * @param mixed                           $parameters
	 */
	private function register_shortcode( \WPCL\CLK\Interfaces\Shortcode_Hook_Subscriber $object, $name, $parameters ) {
		if( is_string( $parameters ) ) {
			// If a class method
			if( method_exists( $object, $parameters ) ) {
	 ...
	 *
	 * @param Shortcode_Hook_Subscriber Interface $object
	 */
	private function register_shortcodes( \WPCL\CLK\Interfaces\Shortcode_Hook_Subscriber $object) {
